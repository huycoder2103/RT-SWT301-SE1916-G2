...
| 2 | LLM (Intervention) | **`claude-sonnet-4-6`** (Claude Sonnet 4.6, Anthropic), temperature=0, top_p=1, max_tokens=4096, seed fixed; few-shot from OpenAPI spec only (black-box, blind to source/faults). Used for **both** the pilot and the full run — no model swap. | #3 (RESTGPT, HUY) — GPT-3.5 pipeline; #9 (MioHint, HUY) — GPT-4o; #13 (APITestGenie, HUY) — GPT-4-Turbo + RAG; team `proposal.md` §5.3 | The proposal originally scoped a GPT-family model (GPT dominates the merged evidence — 47.72pp target-coverage gain #9, 89% script validity #13, 9/13 HUY papers); the team pivoted to **Claude Sonnet 4.6**, the strong closed-source model actually available for the run, so the Week-7 pilot carries over unchanged. A non-GPT model additionally contributes evidence outside the GPT-dominated literature (partial **GAP-T**). See `proposal.md` §5.3. |
...
[Step 1] Endpoint classification
        ...
        ↓
[Step 2] LLM prompt (Claude Sonnet 4.6)
