...
Our evidence base is a five-member systematic review merged into \textbf{59} unique papers
(2018--2026) after de-duplication, screened against pre-registered inclusion/exclusion
...
model~\cite{kat,restspecit,restgpt,autoresttest,logiagent,restifai,apitestgenie};
open-source models appear only sporadically~\cite{llamaresttest}. Our use of Claude
Sonnet 4.6 therefore adds a data point outside the family that supplies almost all current
evidence --- a partial contribution to GAP-T.
...
and only RESTSpecIT~\cite{restspecit} reports 5xx. No study reports edge cases per
endpoint, so there is no baseline against which a suite's negative-testing breadth can be
judged.
...
manual comparison in the field is EvoMaster's own~\cite{evomaster}, which found the tool
well below manual (41\% vs 82\%) --- an inconvenient result that, notably, no LLM paper has
revisited even as the field's rhetoric moved to human parity.

\textbf{4. Fault detection has no denominator.}
Every fault figure in Table~\ref{tab:related} is a count on a live
API~\cite{llamaresttest,autoresttest,logiagent,notimetorest,evomaster,morest}. This is the
deepest methodological problem in the area, because error counts and Recall can be ordered
differently: a suite that hammers one endpoint hard can log many 500s while missing most of
the fault population. Mutation testing is standard elsewhere in software
engineering~\cite{pit,offutt}, and its absence here is what leaves the field's central
comparative claims untestable.

\subsection{Positioning}
Table~\ref{tab:gapmap} maps each gap to the evidence establishing it. Our design targets
GAP-C and GAP-D jointly --- adding both a manual arm and a known fault population is what
makes the three-way comparison meaningful --- and treats GAP-M as the metric facet through
which the result is interpreted.
...
\label{tab:gapmap}
\small
\begin{tabular}{@{}p{1.35cm}p{4.1cm}p{1.9cm}@{}}
\toprule
