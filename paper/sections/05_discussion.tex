...
\textbf{Oracle kind sets a ceiling; volume allocates what is under it.} The pilot behind this
study reported LLM and manual killing identical mutant sets, but its manual suite shared an
author with the LLM suite, so that identity could not be interpreted. Re-authoring the manual
suite independently (spec-only, isolated, blind) separates two effects that the tie had
conflated. On \texttt{rest-ncs}, where every mutant is a value fault, the independent manual
still scores \RQIINcsManual{} --- exactly like the LLM, and for the structural reason the
mechanism predicts: no status-oracle suite, however authored, can see a wrong number behind a
200. Oracle \emph{kind} caps both at zero there. But on \texttt{rest-scs}, where some mutants
flip a response \emph{class} and are therefore status-visible, the suites separate sharply:
the LLM kills \RQIIScsLlm{} to the independent manual's \RQIIScsManual{}, and the manual's
kills are a strict subset of the LLM's. Within the status-oracle regime, the LLM's
\PDtestsLlm{}-test volume reaches more of the catchable faults than the leaner
\PDtestsManual{}-test human-style suite. So the two-level picture is: oracle kind fixes the
\emph{ceiling} (status oracles cannot exceed EvoMaster on value faults), and within a kind,
volume determines how much of the reachable fault set you actually catch. The pilot's tie was
the two levels coinciding by an authorship artefact; separating the author reveals both.
